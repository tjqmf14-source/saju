const CATEGORY_LABELS={overall:'총운',money:'재물',love:'연애',work:'직업',condition:'컨디션'};
const GROUP_AFFINITY={
  비겁:{overall:5,money:-2,love:5,work:4,condition:5},
  식상:{overall:7,money:5,love:6,work:8,condition:2},
  재성:{overall:7,money:12,love:5,work:8,condition:0},
  관성:{overall:6,money:5,love:3,work:12,condition:-2},
  인성:{overall:5,money:1,love:4,work:6,condition:10}
};
...
function natalRoleBonus(chart,category){
  ...
  const map={money:'재성',work:'관성',condition:'인성',love:'비겁'};
...
export function calculateDailyScores(chart,todayFlow){
  ...
  const individual={};
  for(const category of ['money','love','work','condition']){
  ...
  const mean=Object.values(individual).reduce((sum,item)=>sum+item.score,0)/4;
