...
export function buildDetailedInterpretation(chart, mbti, yearFlow, monthFlows){
  ...
  return {
    overview: section(
      ...
      [
        `이 사주를 한 문장으로 줄이면 ‘${element.gift}을 바탕으로, ${role.core}를 삶의 방식으로 만들어 가는 사람’에 가깝습니다. 한 가지 성격표처럼 고정해서 보기보다는, 어떤 상황에서 힘을 얻고 어떤 환경에서 피로가 커지는지를 함께 보는 편이 훨씬 정확합니다. 특히 ${strongElement} 기운이 상대적으로 선명하게 드러나기 때문에 ${element.gift}이 자연스러운 장점이 되기 쉽고, 중요한 선택 앞에서는 ${ROLE_LABELS[strongRole]}의 기준이 반복해서 작동할 가능성이 큽니다.`,
