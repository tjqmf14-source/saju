...
test('product v15 is the only active presentation layer',()=>{
  assert.match(html,/href="\/product-v15\.css"/);
  assert.doesNotMatch(html,/href="\/site-v12\.css"/);
  assert.match(html,/data-theme="product-v15"/);
});

test('commercial UI uses progressive disclosure and mobile-first controls',()=>{
  for(const token of ['--surface:#fff','--accent:#315c48','.mobile-bottom-nav','.full-report-shell','.tarot-fan-stage']){
