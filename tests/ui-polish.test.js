...
const css = await readFile(new URL('../polish.css', import.meta.url), 'utf8').catch(()=>'');
...
test('production change: 폴리싱 스타일시트를 마지막에 로드한다', () => {
  assert.match(html, /<link rel="stylesheet" href="\/polish\.css">/i, expectedChange);
});
...
test('production change: 작은 화면에서 핵심 내비게이션과 레이아웃을 재배치한다', () => {
  assert.match(css, /@media\s*\(max-width:\s*760px\)/i, expectedChange);
...
test('production change: reduced-motion 환경을 존중한다', () => {
  assert.match(css, /prefers-reduced-motion:\s*reduce/i, expectedChange);
});
